...
void DisplayVariablesMenu(GuiDebug* gui) {
  ...
  if (gui->variableEntries.empty() && gui->variableRegister) gui->variableRegister();

  if (gui->variableEntries.empty()) {
    pros::screen::print(pros::E_TEXT_MEDIUM_CENTER, 3, "No Variables Registered");
    return;
  }

  // Calculate pagination
  int totalPages = (gui->variableEntries.size() + VARS_PER_PAGE - 1) / VARS_PER_PAGE;
  ...
  int endIdx = std::min(startIdx + VARS_PER_PAGE, (int)gui->variableEntries.size());
  ...
  for (int i = startIdx; i < endIdx; ++i) {
    const auto& e = gui->variableEntries[i];
...
void HandleVariablesMenuTouch(GuiDebug* gui) {
  ...
  // BACK button
  {
    ...
    if (touch.x >= backX1 && touch.x <= backX2 && touch.y >= backY1 && touch.y <= backY2) {
      gui->CurrentScreen = gui->PreviousScreen;
      variablesPage = 0;  // Reset page when leaving
      if (gui->PreviousScreen == DebugMenu) gui->DisplayDebugMenu();
      lastTouchMs = now;
      pros::delay(300);
      return;
    }
