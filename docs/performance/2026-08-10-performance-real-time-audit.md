...
| Concern | Status | Repository evidence | Decision |
|---|---|---|---|
| Slow compilation | **Conditional** | No build-time baseline exists. `include/aon/tools/json.hpp` is about 19,000 lines and enters through `include/aon/api.hpp:17`; `src/aon/core/robot.cpp:3` reaches that umbrella through `main.hpp` | Capture clean and incremental build times plus include traces before splitting headers |
...
| Header/source split overhead | **Not found** | The project mixes `.hpp` interfaces with `.cpp` implementations in the normal PROS pattern. Manual separation adds maintenance but keeps embedded linking predictable | Prefer clear ownership boundaries; do not convert files merely to follow a language trend |
...
| Fragmented dependency management | **Not found** | `project.pros` pins kernel 4.2.2, LemLib 0.5.6, and LVGL 9.2.0; matching firmware archives are present. No vcpkg or Conan workflow is required | Treat `project.pros` as the dependency manifest; verify templates on a clean machine and record checksums/build provenance |
