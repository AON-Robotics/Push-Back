...
double RDP::perpendicularDistance(const Point& A, const Point& B, const Point& P) {
    double dx = B.x - A.x;
    double dy = B.y - A.y;
...
void RDP::rdpRecursive(const std::vector<Point>& points,
                       int startIndex,
                       int endIndex,
...
std::vector<Point> RDP::simplify(const std::vector<Point>& points) {
    if (points.size() < 2)
        return points;
