...
class XDrive : public Drivetrain {
 private:
  ...
  okapi::MotorGroup backRightMotors;
 ...
 public:
  ...
  /// @param brakeMode The braking paradigm we will use, usually `holding` for auton and `brake` for drivers
  /// @param gearset The gearbox the physical motors contain, they MUST be all the same
  void configure(okapi::AbstractMotor::brakeMode brakeMode, okapi::AbstractMotor::gearset gearset) override;
