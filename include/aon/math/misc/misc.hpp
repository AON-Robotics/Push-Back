...
inline double getAngleOfArc(const Vector &a, const Vector &b, const Vector &center)
{
...
inline double getErrorPercentage(const double &expected, const double &actual) { return ((expected - actual) / expected) * 100; }
...
inline double getPercentDifference(const double &a, const double &b) { return (std::abs(a - b) / ((a + b) / 2)) * 100; }
