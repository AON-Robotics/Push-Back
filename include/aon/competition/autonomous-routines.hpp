...
void testDistanceFromVision(){
  ...
  activateORBITFollow();
  while(true){
    pros::vision_object block = vision_sensor.get_by_sig(0, RED);
    if(block.signature == RED){
      const double distance = groundDistanceToDisk(block.width);
...
#if USING_BIG_ROBOT
void testBigBotRoutine(){
  intake.activateScan();
  strafe(-29.25); // Align with match loader
